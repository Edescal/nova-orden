        ...
        <section id={`producto-id-${producto.id}`} className='bg-light border-bottom border-end border-start p-3' onClick={onDetail ?? null}>
            <div className='d-flex'>
                <div className='ratio ratio-1x1 flex-grow-0 flex-shrink-0 d-flex align-self-center' style={{ flexBasis: 'clamp(70px, 16vw, 140px)' }}  >
                    <img src={producto.imagen ?? noimgfound} alt={producto.imagen ? `Imagen para ${producto.nombre}` : 'Imagen de producto'} className='bg-body-secondary rounded-3 shadow img-fluid object-fit-contain' />
                </div>
